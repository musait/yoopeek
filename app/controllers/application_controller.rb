class ApplicationController < ActionController::Base
  ...
  def configure_permitted_parameters
    ...
    devise_parameter_sanitizer.permit:account_update,keys: [:bank_token, :birthdate, :account_token, :person_token, :email, :password, :password_confirmation, :firstname, :lastname,:is_worker,:description,:profession_id,:phone_number,:price_rate,:banner,:avatar,:facebook_profile,:instagram_profile,:pinterest_profile,:twitter_profile,:category_id,:skills => [],:address_attributes => [:id, :complete_address,:street, :city, :zip, :country],:tag_ids => [],:subcategory_ids => [], :company_attributes => [:id, :website,:name, :iban, :subject_to_vat, :vat_number,:currency, :address_attributes =>[:id, :complete_address,:street, :city, :zip, :country]]]
  end
